...
export async function generateMonthlyReportsLogic({ step }) {
  ...
  for (const user of users) {
    await step.run(`generate-report-${user.id}`, async () => {
      ...
      const html = `<div style="font-family: Arial, sans-serif; padding: 24px;">
        <h2 style="color: #4CAF50;">📊 Monthly Financial Report - ${monthName}</h2>
        <p>Hi <strong>${user.name}</strong>,</p>
        <p>Here is your monthly report for ${monthName}:</p>
        <ul>
          <li><strong>Total Income:</strong> ₹${stats.totalIncome}</li>
          <li><strong>Total Expenses:</strong> ₹${stats.totalExpenses}</li>
          <li><strong>Balance:</strong> ₹${stats.balance}</li>
        </ul>
        <h3>💡 Insights:</h3>
        <p>${insights}</p>
